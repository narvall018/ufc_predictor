...
<style>
    ...
    /* Styles pour l'upload du modèle */
    .upload-box {
...
# Fonction modifiée pour charger le modèle avec plus de robustesse
@st.cache_resource
def load_ml_model(uploaded_file=None):
    """
    Charge le modèle ML entraîné, le scaler et les feature names
    avec plusieurs méthodes pour maximiser les chances de succès
    """
    # Si un fichier a été uploadé, l'utiliser en priorité
    if uploaded_file is not None:
        try:
            ...
            if uploaded_file.name.endswith('.joblib'):
                model_data = joblib.load("temp_model_file")
            else:
                with open("temp_model_file", "rb") as f:
                    model_data = pickle.load(f)
            
            st.sidebar.success("✅ Modèle chargé avec succès depuis le fichier uploadé!")
            ...
            st.sidebar.error(f"Erreur lors du chargement du fichier uploadé: {e}")
            return None, None, None
    
    # Essayer de charger à partir des chemins possibles
    possible_paths = [
        "ufc_prediction_model.joblib",
        "ufc_prediction_model.pkl",
        "models/ufc_prediction_model.joblib",
        "models/ufc_prediction_model.pkl",
        "./ufc_prediction_model.joblib",
        "./ufc_prediction_model.pkl",
    ]
    
    # Rechercher le modèle dans tous les chemins possibles
    for path in possible_paths:
        try:
            if path.endswith('.joblib') and os.path.exists(path):
                model_data = joblib.load(path)
                model = model_data.get('model')
                scaler = model_data.get('scaler')
                feature_names = model_data.get('feature_names')
                
                st.sidebar.success(f"✅ Modèle chargé depuis {path}")
                return model, scaler, feature_names
            elif path.endswith('.pkl') and os.path.exists(path):
            ...
            st.sidebar.warning(f"Échec du chargement depuis {path}: {e}")
            continue
    
    # Si on est ici, aucun modèle n'a été chargé
    st.sidebar.warning("⚠️ Aucun modèle ML trouvé.")
    st.sidebar.info("💡 Vous pouvez uploader manuellement votre modèle ou utiliser uniquement les prédictions statistiques.")
    
    # Liste des fichiers dans le répertoire courant pour le débogage
    try:
        files_in_dir = os.listdir('.')
        st.sidebar.info(f"📁 Fichiers dans le répertoire courant: {', '.join(files_in_dir)}")
    except Exception as e:
        st.sidebar.error(f"Erreur lors de la liste des fichiers: {e}")
...
# Fonction pour intégrer le modèle ML dans l'app
def embed_model_in_streamlit(model, scaler, feature_names):
    """
    Encode le modèle ML pour l'intégrer dans l'app Streamlit
    """
    try:
        ...
        with open("temp_model_to_encode.joblib", "wb") as f:
            joblib.dump(model_data, f)
        
        # Lire le fichier et l'encoder en base64
        with open("temp_model_to_encode.joblib", "rb") as f:
            model_bytes = f.read()
            model_b64 = base64.b64encode(model_bytes).decode()
        
        # Supprimer le fichier temporaire
        os.remove("temp_model_to_encode.joblib")
        
        return model_b64
    except Exception as e:
        st.error(f"Erreur lors de l'encodage du modèle: {e}")
        return None
...
def main():
    ...
    # Option d'upload du modèle dans la sidebar
    st.sidebar.markdown("## Configuration avancée")
    
    uploaded_model = st.sidebar.file_uploader(
        "📤 Charger le modèle ML (.joblib ou .pkl)", 
        type=["joblib", "pkl"], 
        help="Si vous disposez du fichier modèle, vous pouvez le charger ici"
    )
    
    # Zone d'explication sur l'upload du modèle
    if uploaded_model is None:
        st.sidebar.markdown("""
        <div class="upload-box">
            <p>📌 <b>Besoin du modèle ML?</b></p>
            <p>Si vous avez le fichier du modèle ML, uploadez-le ci-dessus pour activer les prédictions par Machine Learning.</p>
            <p>Noms de fichiers acceptés:<br>
            • <code>ufc_prediction_model.joblib</code><br>
            • <code>ufc_prediction_model.pkl</code></p>
        </div>
        """, unsafe_allow_html=True)
    
    # Chargement du modèle ML
    model, scaler, feature_names = load_ml_model(uploaded_model)
    ...
    if predict_btn and fighter_a and fighter_b:
        ...
        # Explication pour l'upload du modèle
        if model is None:
            st.markdown("""
            <div style="background-color:rgba(33, 150, 243, 0.1); padding:15px; border-radius:10px; margin-top:20px;">
                <h3>🔧 Besoin du modèle ML pour activer toutes les fonctionnalités</h3>
                <p>Pour utiliser les prédictions par Machine Learning, vous avez deux options:</p>
                <ol>
                    <li>Placez le fichier du modèle <code>ufc_prediction_model.joblib</code> ou <code>ufc_prediction_model.pkl</code> dans le même répertoire que cette application.</li>
                    <li>Utilisez la fonction d'upload dans le menu latéral pour téléverser manuellement le fichier modèle.</li>
                </ol>
                <p>Sans le modèle ML, vous pouvez toujours utiliser les prédictions basées sur les statistiques.</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        if model is not None:
            ...
            <div style="background-color:rgba(244, 67, 54, 0.1); padding:15px; border-radius:10px; margin-top:20px;">
                ...
                <p>Le modèle de machine learning n'a pas été trouvé. Vous pouvez:</p>
                <ul>
                    <li>Utiliser la fonction d'upload dans le sidebar pour charger manuellement le modèle</li>
                    <li>Placer le fichier <code>ufc_prediction_model.joblib</code> ou <code>ufc_prediction_model.pkl</code> dans le même répertoire que cette application</li>
                </ul>
                <p>Seule la méthode de prédiction statistique classique sera disponible sans le modèle ML.</p>
            </div>
            """, unsafe_allow_html=True)

        # Si un modèle est chargé, ajouter une option pour le télécharger et le partager
        if model is not None and scaler is not None and feature_names is not None:
            st.markdown("### 🔄 Partager le modèle ML")
            
            if st.button("Générer un fichier modèle à partager"):
                model_b64 = embed_model_in_streamlit(model, scaler, feature_names)
                if model_b64:
                    # Créer un lien de téléchargement pour le modèle
                    st.download_button(
                        label="Télécharger le modèle",
                        data=base64.b64decode(model_b64),
                        file_name="ufc_prediction_model.joblib",
                        mime="application/octet-stream"
                    )
                    
                    st.success("✅ Modèle prêt à être téléchargé et partagé!")
